...
final class CameraConfigurationManager {
    ...
    void setDesiredCameraParameters(Camera camera) {
        Camera.Parameters parameters = camera.getParameters();
        Log.d(TAG, "Setting preview size: " + cameraResolution);// Point(1920, 1080)
...
//         camera.setDisplayOrientation(90);
        ...
        Log.e("cccccc",cameraResolution.x+"width--- height->"+cameraResolution.y);
    ...
    private static Point findBestPreviewSizeValue(
        ...
        Log.e("ccc",COMMA_PATTERN.split(previewSizeValueString).length+"");
        for (int i = 0; i < COMMA_PATTERN.split(previewSizeValueString).length; i++) {
            Log.e("ccc",COMMA_PATTERN.split(previewSizeValueString)[i]);
        }
...
//            int newDiff = Math.abs(newX - screenResolution.x) + Math.abs(newY - screenResolution.y);
            int newDiff=Math.abs(newY - screenResolution.x) + Math.abs(newX - screenResolution.y);//修改二维码变形问题
            if (newDiff == 0) {
                bestX = newX;
                bestY = newY;
                break;
